...
# puts "***********************************"
          ...
          set_error 999, "Bad call" and return if @fun.nil?
          #@fun = @fun[0]
          return unless @req.env['QUERY_STRING']
          @req.env['QUERY_STRING'].split('&').each do |pair|
            arg, val = pair.split("=")
          ...
          # Note that no suppression of 'nil' and 'false' is made for GET requests (only for POSTs)
        end
      end
